...
const areAdjacent = (a, b) => adjacent(a).find((c) => c.x === b.x && c.y === b.y)

const moveTail = (head, tail) =>
    areAdjacent(tail, head) ? tail : {
...
// parse input file
const motions = file.split('\n')
    .filter(x => x)
    .map(row => row.split(' '))
    .map(([direction, distance]) => ({ direction, vector: directionToVector(direction), distance: Number(distance) }))

let knotPositions = Array(NUM_KNOTS).fill({x: 0, y: 0})
const knotPaths = [...Array(NUM_KNOTS).keys()].map(() => new Set())

motions.forEach(motion => {
    const previousKnotPositions = knotPositions.slice()
